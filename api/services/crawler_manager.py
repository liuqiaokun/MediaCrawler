...
class CrawlerManager:
    ...
    def __init__(self):
        ...
        load_dotenv(self._project_root / ".env", override=False)
    ...
    def get_status(self) -> dict:
        """Get current status"""
        if self.process and self.process.poll() is not None and self.status in {"running", "stopping"}:
            self.status = "idle" if self.process.returncode == 0 else "error"
        ...
        return {
            ...
            "error_message": None if not self.process or self.process.returncode in (None, 0) else f"Process exited with code {self.process.returncode}"
        }

    @staticmethod
    def _decode_output(raw: bytes) -> str:
        """Decode subprocess output robustly across Windows locale differences."""
        if not raw:
            return ""

        encodings = ["utf-8", "utf-8-sig", locale.getpreferredencoding(False), "gb18030", "gbk"]
        tried = set()
    ...
    def _build_command(self, config: CrawlerStartRequest) -> list:
        """Build run_keyword_monitor.py command line arguments"""
        if sys.platform == "win32":
            python_exe = self._project_root / ".venv" / "Scripts" / "python.exe"
        else:
            python_exe = self._project_root / ".venv" / "bin" / "python"
        runner_script = self._project_root / "scripts" / "run_keyword_monitor.py"
        ...
        cmd = [
            ...
            "--get-comment",
            "true" if config.enable_comments else "false",
            "--get-sub-comment",
            "true" if config.enable_sub_comments else "false",
            "--headless",
            "true" if config.headless else "false",
            "--max-notes-count",
            str(config.max_notes_count),
            "--max-comments-count-singlenotes",
            str(config.max_comments_count_singlenotes),
            "--max-concurrency-num",
            str(config.max_concurrency_num),
            "--run-name",
            config.run_name or f"webui_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        ]
        ...
        if config.enable_ip_proxy:
            cmd.append("--enable-ip-proxy")
            cmd.extend(["--ip-proxy-provider-name", config.ip_proxy_provider_name])
    ...
    async def _read_output(self):
        ...
        try:
            while self.process and self.process.poll() is None:
                ...
                if raw_line:
                    line = self._decode_output(raw_line).strip()
                    if line:
            ...
            if self.process and self.process.stdout:
                ...
                if remaining:
                    decoded = self._decode_output(remaining)
            ...
            if self.status == "running":
                ...
                await self._push_log(entry)
                self.status = "idle" if exit_code == 0 else "error"
