...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def search_by_keywords(self):
        ...
        target_count = max(config.CRAWLER_MAX_NOTES_COUNT, 0)
        if target_count <= 0:
            utils.logger.info("[BilibiliCrawler.search_by_keywords] Skip search because max notes count is 0")
            return
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while collected_count < target_count:
                ...
                utils.logger.info(
                    f"[BilibiliCrawler.search_by_keywords] search bilibili keyword: {keyword}, page: {page}, remaining: {remaining}"
                )
                ...
                video_items = await self.get_video_items_for_page(video_list, remaining, semaphore)
                ...
                utils.logger.info(
                    f"[BilibiliCrawler.search_by_keywords] Collected {collected_count}/{target_count} videos for keyword: {keyword}"
                )
                ...
                if collected_count >= target_count:
                    utils.logger.info(
                        f"[BilibiliCrawler.search_by_keywords] Reached target video count: {target_count}"
                    )
                    break

    async def get_video_items_for_page(
        self,
        video_list: List[Dict],
        remaining_count: int,
        semaphore: asyncio.Semaphore,
    ) -> List[Dict]:
        """Fetch just enough video details from a page to satisfy the remaining target count."""
        video_items: List[Dict] = []
        start_index = 0

        while start_index < len(video_list) and len(video_items) < remaining_count:
            batch_size = remaining_count - len(video_items)
            current_batch = video_list[start_index:start_index + batch_size]
            try:
                task_list = [
                    self.get_video_info_task(aid=video_item.get("aid"), bvid="", semaphore=semaphore)
                    for video_item in current_batch
                ]
            except Exception as e:
                utils.logger.warning(
                    f"[BilibiliCrawler.get_video_items_for_page] error in the task list. "
                    f"The remaining videos for this batch will not be included. {e}"
                )
                break

            batch_results = await asyncio.gather(*task_list)
            video_items.extend([video_item for video_item in batch_results if video_item])
            start_index += len(current_batch)

        return video_items[:remaining_count]
