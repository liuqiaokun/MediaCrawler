...
class DouYinCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        if config.ENABLE_IP_PROXY:
            ...
            utils.logger.info(
                "[DouYinCrawler] 浏览器登录页使用直连，接口请求继续走代理，以减少二维码加载失败概率"
            )
        ...
        async with async_playwright() as playwright:
            ...
            existing_pages = [page for page in self.browser_context.pages if not page.is_closed()]
            if existing_pages:
                self.context_page = existing_pages[0]
            ...
            first_keyword = next(
                (keyword.strip() for keyword in config.KEYWORDS.split(",") if keyword.strip()),
                "",
            )
            if first_keyword:
                search_url = f"https://www.douyin.com/search/{urllib.parse.quote(first_keyword)}?type=general"
                utils.logger.info(f"[DouYinCrawler] 登录后跳转到搜索页: {search_url}")
                await self.context_page.goto(
                    search_url,
                    wait_until="domcontentloaded",
                    timeout=60000,
    ...
    async def launch_browser(
        ...
        if config.SAVE_LOGIN_STATE:
            user_data_dir_name = config.USER_DATA_DIR % config.PLATFORM  # type: ignore
