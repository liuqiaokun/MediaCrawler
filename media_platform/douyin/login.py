...
class DouYinLogin(AbstractLogin):
    ...
    async def begin(self):
        ...
        try:
            ...
            await self._send_wechat_notification("抖音登录失败：Cookie 已过期或扫码超时，请重新扫码登录")
            sys.exit()
    ...
    async def popup_login_dialog(self):
        ...
        full_panel_selector = "xpath=//div[starts-with(@id,'login-full-panel-')]"
        try:
            # check dialog box is auto popup and wait for 20 seconds
            # try both the old #login-panel-new and the new login-full-panel-* selectors
            old_panel = self.context_page.locator(dialog_selector)
            new_panel = self.context_page.locator(full_panel_selector)
            await old_panel.or_(new_panel).first.wait_for(state="visible", timeout=1000 * 20)
            utils.logger.info("[DouYinLogin.popup_login_dialog] login dialog detected")
        except Exception as e:
            utils.logger.error(f"[DouYinLogin.popup_login_dialog] login dialog box does not pop up automatically, error: {e}")
            # check if the full panel is already in the DOM (even if wait timed out due to race condition)
            full_panel_count = await self.context_page.locator(full_panel_selector).count()
            if full_panel_count > 0:
                utils.logger.info("[DouYinLogin.popup_login_dialog] login-full-panel detected, dialog is already open")
            else:
                utils.logger.info("[DouYinLogin.popup_login_dialog] no login panel found, manually clicking login button")
    ...
    async def login_by_qrcode(self):
        ...
        base64_qrcode_img = await utils.find_login_qrcode(
            ...
            timeout=self.scan_qrcode_time,
        )
        if not base64_qrcode_img:
            await self._save_debug_screenshot("qrcode_not_found")
            utils.logger.info("[DouYinLogin.login_by_qrcode] login qrcode not found please confirm ...")
            await self._send_wechat_notification("抖音登录失败：二维码加载失败，请检查网络或手动登录")
            sys.exit()
    ...
    async def _save_debug_screenshot(self, reason: str) -> None:
        run_tag = os.getenv("MEDIACRAWLER_RUN_TAG", "manual")
        debug_dir = os.path.join(os.getcwd(), "workspace", "debug")
        os.makedirs(debug_dir, exist_ok=True)
        screenshot_path = os.path.join(debug_dir, f"dy_login_{run_tag}_{reason}.png")
        try:
            await self.context_page.screenshot(path=screenshot_path, full_page=True)
            utils.logger.info(f"[DouYinLogin] saved debug screenshot: {screenshot_path}")
        except Exception as exc:
            utils.logger.error(f"[DouYinLogin] save debug screenshot failed: {exc}")

    @staticmethod
    async def _send_wechat_notification(message: str) -> None:
        """Send a notification to WeChat Work group via webhook"""
        webhook_url = getattr(config, "WECHAT_WEBHOOK_URL", "")
        if not webhook_url:
            utils.logger.warning("[DouYinLogin] WECHAT_WEBHOOK_URL not configured, skip notification")
            return
        payload = {"msgtype": "text", "text": {"content": message}}
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.post(webhook_url, json=payload, timeout=10)
            utils.logger.info(f"[DouYinLogin] WeChat notification sent, status={resp.status_code}")
        except Exception as e:
            utils.logger.error(f"[DouYinLogin] Failed to send WeChat notification: {e}")
