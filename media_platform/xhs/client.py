...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        ...
        try:
            ...
            raise DataFetchError(f"HTTP transport error: {e}")
        ...
        if response.status_code == 471 or response.status_code == 461:
            ...
            msg = (
                "[XiaoHongShuClient.request] CAPTCHA or risk-control response detected, "
                f"status_code={response.status_code}, method={method}, url={url}, "
                f"verify_type={verify_type}, verify_uuid={verify_uuid}, "
                f"response_headers={json.dumps(interesting_headers, ensure_ascii=False)}, "
