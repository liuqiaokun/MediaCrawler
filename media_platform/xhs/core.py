...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def search(self) -> None:
        ...
        if target_count <= 0:
            utils.logger.info("[XiaoHongShuCrawler.search] Skip search because max notes count is 0")
            return
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while collected_count < target_count:
                ...
                try:
                    ...
                    utils.logger.info(
                        f"[XiaoHongShuCrawler.search] search Xiaohongshu keyword: {keyword}, page: {page}, remaining: {remaining}"
                    )
                    note_ids: List[str] = []
                    ...
                    note_details = await self.get_note_details_for_page(post_items, remaining, semaphore)
                    ...
                    utils.logger.info(
                        f"[XiaoHongShuCrawler.search] Collected {collected_count}/{target_count} notes for keyword: {keyword}"
                    )
                    ...
                    if collected_count >= target_count:
                        utils.logger.info(
                            f"[XiaoHongShuCrawler.search] Reached target note count: {target_count}"
                        )
                        break
    ...
    async def get_note_details_for_page(
        self,
        post_items: List[Dict],
        remaining_count: int,
        semaphore: asyncio.Semaphore,
    ) -> List[Dict]:
        """Fetch just enough note details from a page to satisfy the remaining target count."""
        note_details: List[Dict] = []
        start_index = 0

        while start_index < len(post_items) and len(note_details) < remaining_count:
            batch_size = remaining_count - len(note_details)
            current_batch = post_items[start_index:start_index + batch_size]
            task_list = [
                self.get_note_detail_async_task(
                    ...
                    semaphore=semaphore,
                ) for post_item in current_batch
            ]
            batch_details = await asyncio.gather(*task_list)
            note_details.extend([note_detail for note_detail in batch_details if note_detail])
            start_index += len(current_batch)

        return note_details[:remaining_count]
